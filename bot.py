        ...
        try:
            ...
            def escape_markdown(text):
             return text.replace('`', '')            
              ...
              # Save pretty text tables to a safe path
            try:
                output_dir = "/home/site/wwwroot/outputs"
                os.makedirs(output_dir, exist_ok=True)  # Create outputs folder if missing

                table_output_path = os.path.join(output_dir, "table_df_output_telegram.json")
                summary_output_path = os.path.join(output_dir, "summary_df_output_telegram.json")

                # Save DataFrame df as JSON
                df.to_json(table_output_path, orient="records", indent=4)

                # Save DataFrame df2 as JSON
                df2.to_json(summary_output_path, orient="records", indent=4)

                logging.info(f"Saved output JSON files at {output_dir}")
            except Exception as file_error:
                logging.error(f"Error saving to text files: {file_error}")

            # Load your JSON list (summary)
            with open("/home/site/wwwroot/outputs/table_df_output_telegram.json", "r", encoding="utf-8") as f:
                data_header = json.load(f)

            # Load your JSON list (table)
            with open("/home/site/wwwroot/outputs/summary_df_output_telegram.json", "r", encoding="utf-8") as f:
                data = json.load(f)

            await turn_context.send_activity("data loading completed")
            await turn_context.send_activity("generating output..")
            ...
            # Build the message
            message = "**Extracted headers:**\n\n"

            for item in data_header[:6]:  # Only first 6 entries
                field_value = escape_markdown(str(item.get("Field Value", "")))
                field_name = escape_markdown(str(item.get("Field Name", "")))
                message += f" `{field_name}` : `{field_value}`\n\n"

            # Send message
            await turn_context.send_activity(message)

            # first part of table content
            message2 = "**Extracted table information:**\n\n"

            for item in data[:1]:  # Only table details
                quantity = escape_markdown(str(item.get("Quantity", "")))
                Description = escape_markdown(str(item.get("Description", "")))
                UnitPrice = escape_markdown(str(item.get("Unit Price", "")))
                Amount = escape_markdown(str(item.get("Amount", "")))
                message2 += f"Quantity :`{quantity}`\n\n"
                message2 += f"Description :`{Description}`\n\n"
                message2 += f"Unit Price  :`{UnitPrice}`\n\n"
                message2 += f"Amount      :`{Amount}`\n\n" 

            # Send message
            await turn_context.send_activity(message2)   

             # second part of table content
            message3 = "**Extracted sub information:**\n\n"

            for item in data[2:]: # Start from 2nd index until the end
                Total = escape_markdown(str(item.get("Total", "")))
                Amount = escape_markdown(str(item.get("Amount", "")))
                message3 += f" `{Total}` : `{Amount}`\n\n"
